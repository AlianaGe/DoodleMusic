...
class AccoMontage:
    ...
    def __init__(self, midi, segmentation, note_shift=0, spotlight=[], prefilter=(4, 1),
        ...
        # if self.phrase_data is not None:
        #     Logging.info('preLoading AccoMontage')
        #     AccoMontage.data = self.phrase_data
        #     if AccoMontage.melody1 is None:
        #         AccoMontage.melody1 = AccoMontage.data['melody']
        #         AccoMontage.acc1 = AccoMontage.data['acc']
        #         AccoMontage.chord1 = AccoMontage.data['chord']
        #         AccoMontage.acc_pool1 = {}
        #         for i in [8, 6, 4]:
        #             (mel, acc_, chord_, song_reference) = \
        #                 find_by_length(AccoMontage.melody1, AccoMontage.acc1, AccoMontage.chord1, i)
        #             AccoMontage.acc_pool1[i] = (mel, acc_, chord_, song_reference)
        #     Logging.info('Loading AccoMontage done!')
        # # if AccoMontage.data is None:
        # #     AccoMontage.data = np.load(DATA_DIR + '/phrase_data0714.npz', allow_pickle=True)
        ...
        if AccoMontage.data is None:
            ...
            for i in [8, 6, 4]:
                ...
                AccoMontage.acc_pool1[i] = (mel, acc_, chord_, song_reference)
            Logging.info('Loading AccoMontage done!')
        # elif AccoMontage.data is None:
        #     # print("--------Global------------LoadingHHHHHHData--------------")
        #     # AccoMontage.data = np.load(DATA_DIR + '/phrase_data0714.npz', allow_pickle=True)
        #     AccoMontage.melody1 = AccoMontage.data['melody']
        #     AccoMontage.acc1 = AccoMontage.data['acc']
        #     AccoMontage.chord1 = AccoMontage.data['chord']
    ...
    def solve(self):
        ...
        Logging.warning('1.1')  # if not self.phrase_data:
        melody = AccoMontage.melody1
        acc = AccoMontage.acc1
        chord = AccoMontage.chord1
        acc_pool = {}
        ...
        acc_pool = AccoMontage.acc_pool1
        ...
        midi.instruments.append(self.midi.instruments[1])
